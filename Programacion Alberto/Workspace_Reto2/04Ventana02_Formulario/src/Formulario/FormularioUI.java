...
public class FormularioUI {
	...
	public static void main(String[] args) {
		...
		JTextField textApellidoUno = new JTextField();
		...
		JTextField textApellidoDos = new JTextField();
		...
		btnEnviar.setBounds(142, 395, 99, 38);
		...
		btnEnviar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String apellidoUno = textApellidoUno.getText();
				String apellidoDos = textApellidoDos.getText();
				
				String genero = "";
	            if (rdbtnGeneroMasculino.isSelected()) {
	                genero = "Masculino";
	            } else if (rdbtnGeneroFemenino.isSelected()) {
	                genero = "Femenino";
	            } else if (rdbtnGeneroApache.isSelected()) {
	                genero = "Helicóptero Apache";
	            }
				
	            ArrayList<String> gustos = new ArrayList<>();
	            if(chckbxBailar.isSelected()) gustos.add("Bailar");
	            if(chckbxCantar.isSelected()) gustos.add("Cantar");
	            if(chckbxCine.isSelected()) gustos.add("Cine");
	            if(chckbxComer.isSelected()) gustos.add("Comer");
	            if(chckbxDormir.isSelected()) gustos.add("Dormir");
	            if(chckbxLeer.isSelected()) gustos.add("Leer");
	            if(chckbxMusica.isSelected()) gustos.add("Musica");
	            if(chckbxVideojuegos.isSelected()) gustos.add("Videojuegos");
	            
	            Persona usuario = Formulario.crearUsuario(nombre, apellidoUno, apellidoDos, genero, gustos);
