	...
	<c:forEach var="producto" items="${listaProducto}">
		<div>
			...
			<form action="listadoProducto" method="post">
				<button type="submit" name="id" value="${producto.id}">Añadir
					al carro</button>
			</form>

			<form action="detalles" method="get">
				<input type="hidden" name="id" value="${producto.id}"> <input
					type="hidden" name="rutaImagen" value="${producto.rutaImagen}">
				<input type="hidden" name="descripcionLarga"
					value="${producto.descripcionLarga}"> <input type="hidden"
